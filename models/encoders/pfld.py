...
if __name__ == '__main__':
    ...
    ckpt = torch.load("/mnt/D4AEE2D5AEE2AF64/ML-DS/GANs/gan_latent/pretrained/checkpoint.pth.tar", map_location="cpu")
    print(pfld_backbone.load_state_dict(ckpt['pfld_backbone']))
    print(auxiliarynet.load_state_dict(ckpt['auxiliarynet']))

    img_path = "/mnt/D4AEE2D5AEE2AF64/ML-DS/GANs/datasets/CelebAMask-HQ/CelebA-HQ-img/10005.jpg"
    transform1 = transforms.Compose([
        transforms.ToTensor(),  # range [0, 255] -> [0.0,1.0] and convert [H,W,C] to [C,H,W]
        transforms.Resize((112, 112)),
    ])
    img = cv2.imread(img_path)
    img = transform1(img).unsqueeze(0)
